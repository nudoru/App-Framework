...
var debounce = function(func, threshold, execAsap) {
    ...
    toggleClassOnElement: function(className, el) {
        if (el.classList) el.classList.toggle(className); else {
            var classes = el.className.split(" "), existingIndex = classes.indexOf(className);
...
APP = function(global, rootView) {
    ...
    function render() {
        setCurrentViewPortSize(), setCurrentViewPortScroll(), defineViewElements(), initializeComponents(), 
    ...
    function updateAppTitle() {
        _mainHeaderEl.find("h1").html(_appGlobals.appConfig.title), document.title = StringUtils.removeTags(_appGlobals.appConfig.title);
    }
    function defineViewElements() {
        _appContainerEl = $("#app__container"), _appEl = $("#app__contents"), _mainScrollEl = _appEl, 
        _drawerEl = $("#drawer"), _drawerToggleButtonEl = $(".header__drawer-toggle button"), 
        _mainHeaderEl = $("#header"), _mainFooterEl = $("#footer"), _mainSearchInputEl = $(".grid__header .grid__header-search input"), 
        _searchHeaderEl = $(".grid__header h1"), _clearAllButtonEl = $("#clearall-button");
    }
    ...
    function configureUIStreams() {
        var uiresizestream = Rx.Observable.fromEvent(window, "resize"), uiscrollscream = Rx.Observable.fromEvent(_mainScrollEl[0], "scroll");
        ...
        }), _browserScrollStream = Rx.Observable.fromEvent(_mainScrollEl[0], "scroll").throttle(100).subscribe(function() {
            handleViewPortScroll();
        }), _searchInputStream = Rx.Observable.fromEvent(_mainSearchInputEl[0], "keyup").throttle(150).map(function(evt) {
            return evt.target.value;
        }).subscribe(function(value) {
            _eventDispatcher.publish(APP.Events.SEARCH_INPUT, value);
        }), _clearAllButtonStream = Rx.Observable.fromEvent(_clearAllButtonEl[0], _appGlobals.mouseClickEvtStr).subscribe(function() {
            _eventDispatcher.publish(APP.Events.VIEW_ALL_FILTERS_CLEARED);
        }), _drawerToggleButtonStream = Rx.Observable.fromEvent(_drawerToggleButtonEl[0], _appGlobals.mouseClickEvtStr).subscribe(function() {
            toggleDrawer();
        });
    }
    ...
    function disablePointerEventsOnScroll() {
        ...
        $("body").addClass("ignore-pointer-events"), _disablePointerEventsOnScrollTimerStream = Rx.Observable.timer(250).pluck("interval").take(1).subscribe(function() {
    ...
    function positionUIElements() {
        TweenMax.to(_mainHeaderEl, 0, {
            ...
            top: _currentViewPortSize.height + _currentViewPortScroll.top - _mainFooterEl.height()
        });
    }
    ...
    function setFreeTextFilterValue(str) {
        _mainSearchInputEl[0].value = str, _eventDispatcher.publish(APP.Events.SEARCH_INPUT, str);
    }
    ...
    function removeLoadingMessage() {
        TweenMax.to($("#initialization__cover"), 1, {
            alpha: 0,
            ease: Quad.easeOut,
            onComplete: function() {
                $("#initialization__cover").remove();
            }
        }), TweenMax.to($(".initialization__message"), 2, {
            top: "+=50px",
            ease: Quad.easeIn,
            onComplete: function() {
    ...
    function initialize(elID) {
        _containerEl = $(elID), _currentTags = [], _tagTemplate = _.template('<div class="tag"><%= tag %></div>'), 
        hideBar();
    }
