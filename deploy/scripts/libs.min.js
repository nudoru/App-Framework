...
APP = function(global, rootView) {
    ...
    function initializeComponents() {
        ...
        _itemDetailView = _self.ItemDetailView, _itemDetailView.initialize("#details"), 
        _tagBarView = _self.TagBarView, _tagBarView.initialize("tagbar__container"), TweenMax.to(_drawerEl, 0, {
            x: -1 * _drawerWidth
        });
    }
    ...
    function initialize() {
        ...
        _eventDispatcher = APP.EventDispatcher, _isVisible = !0, _modalCoverEl = $("#modal__cover"), 
        _modalBackgroundEl = $(".modal__background"), _modalCloseButtonEl = $(".modal__close-button");
        var modalBGClick = Rx.Observable.fromEvent(_modalBackgroundEl[0], appGlobals.mouseClickEvtStr), modalButtonClick = Rx.Observable.fromEvent(_modalCloseButtonEl[0], appGlobals.mouseClickEvtStr);
    ...
    function add(title, message, button) {
        ...
        return $(_containerEl).prepend(newToast.html), newToast.index = _children.length, 
        newToast.element = $("#" + newToast.id), newToast.height = newToast.element.innerHeight(), 
        newToast.closeBtnStream = Rx.Observable.fromEvent(newToast.element.find("button")[0], "click"), 
        newToast.expireTimeStream = Rx.Observable.interval(_defaultExpireDuration), newToast.lifeTimeStream = Rx.Observable.merge(newToast.closeBtnStream, newToast.expireTimeStream).take(1).subscribe(function() {
            remove(newToast.id);
    ...
    function onTransitionOutComplete(el) {
        var toastIdx = getToastIndexByID(el.attr("id")), toast = _children[toastIdx];
        el.remove(), _children[toastIdx] = null, _children.splice(toast.index, 1), reindex();
    }
    function reindex() {
    ...
    function showItem(item) {
        _currentItem = item, _containerEl.html(_itemDTemplate(_currentItem)), _floatImageView.apply(_containerEl.find(".details__content-preview-images")), 
        _shareButtonEl = document.getElementById("js__content-share-button"), APP.globals().mobile.any() ? $(_shareButtonEl).hide() : _shareButtonEl.addEventListener(APP.globals().mouseClickEvtStr, doShareAction, !1), 
    ...
    function showMessage(obj) {
        _containerEl.html(_messageTemplate(obj)), TweenMax.to(_containerEl, .25, {
            autoAlpha: 1,
            ease: Quad.easeOut,
            delay: .1
        });
    ...
    function hide() {
        _currentItem = null, _floatImageView.remove(_containerEl.find(".preview-images")), 
    ...
    function initialize() {
        _viewPortCoverEl = $(_coverDivID), _captionEl = _viewPortCoverEl.find(".floatimage__caption"), 
        hideFloatImageCover(), _viewPortCoverClickStream = Rx.Observable.fromEvent(_viewPortCoverEl[0], APP.globals().mouseClickEvtStr).subscribe(function() {
            hideFloatImageCover();
        });
    }
    function apply(container) {
        getFloatingElementsInContainer(container).forEach(function(el) {
            $(el).wrapInner('<div class="floatimage__wrapper" />'), el.addEventListener(APP.globals().mouseClickEvtStr, onImageClick, !1);
        });
    }
    ...
    function showImage(imageEl) {
        _currentImageElement = "div" === imageEl.tagName.toLowerCase() ? $(imageEl).find("img") : $(imageEl);
        var imgTargetX, imgTargetY, imgTargetWidth, imgTargetHeight, vpFill = .75, imgSrc = _currentImageElement.attr("src"), imgAlt = _currentImageElement.attr("alt"), imgWidth = _currentImageElement.width(), imgHeight = _currentImageElement.height(), imgPosition = _currentImageElement.offset(), imgRatio = imgWidth / imgHeight, imgTargetScale = 1, vpWidth = _window.width(), vpHeight = _window.height(), vpScrollTop = _window.scrollTop(), vpScrollLeft = _window.scrollLeft(), vpRatio = vpWidth / vpHeight, imgOriginX = imgPosition.left - vpScrollLeft, imgOriginY = imgPosition.top - vpScrollTop;
        ...
        zoomImage.css({
            ...
            width: imgWidth,
            height: imgHeight
        }), _viewPortCoverEl.append(zoomImage), TweenMax.to(_currentImageElement, .25, {
            alpha: 0,
            ease: Circ.easeOut
            ...
            y: imgTargetY,
            ease: Circ.easeOut
        }), showFloatImageCover(), _captionEl.html(imgAlt.length >= 1 ? "<p>" + imgAlt + "</p>" : "");
    }
    function remove(container) {
        return container ? void getFloatingElementsInContainer(container).forEach(function(el) {
            $(el).unwrap(), el.removeEventListener("click", onImageClick);
        }) : void console.log("[FloatingImagesView] nothing to remove from");
    }
    function validateFloatingContainer(container) {
        return container instanceof jQuery ? container : (console.log("[FloatingImagesView] Container is not a jQuery object"), 
        null);
    }
    ...
    var _viewPortCoverEl, _viewPortCoverClickStream, _captionEl, _currentImageElement, _coverDivID = "#floatimage__cover", _floatingImageClass = ".floatimage__srcimage", _zoomedImageClass = "floatimage__zoomedimage", _window = $(window);
    return {
        initialize: initialize,
        apply: apply,
        remove: remove
    ...
    function add(tag) {
        var tagel = $(_tagTemplate({
            tag: tag
        }));
        _containerEl.append(tagel), _currentTags.push({
            label: tag,
            el: tagel
        }), TweenMax.from(tagel, .5, {
            alpha: 0,
            y: "15px",
            ease: Quad.easeOut
        });
    ...
    function remove(tag) {
        ...
        rmv && (rmv.el.remove(), _currentTags.splice(_currentTags.indexOf(rmv), 1));
    }
    function removeAll() {
