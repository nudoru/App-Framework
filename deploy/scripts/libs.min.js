...
APP = function(global, rootView) {
    ...
    function defineViewElements() {
        _appContainerEl = $("#app-container"), _appEl = $("#app"), _mainScrollEl = _appEl, 
        _drawerEl = $("#drawer"), _drawerToggleButtonEl = $(".drawer-toggle button"), _mainHeaderEl = $("#main-header"), 
        _mainFooterEl = $("#main-footer"), _mainSearchInputEl = $(".item-grid-header .search input"), 
        _searchHeaderEl = $(".item-grid-header h1"), _clearAllButtonEl = $("#clearall-button");
    }
    function initializeComponents() {
        _toastView = _self.ToastView, _toastView.initialize("#toast-container"), _modalCoverView = _self.ModalCoverView, 
        ...
        _itemDetailView = _self.ItemDetailView, _itemDetailView.initialize("#item-details-box"), 
        _tagBarView = _self.TagBarView, _tagBarView.initialize("#tag-bar"), TweenMax.to(_drawerEl, 0, {
            x: -1 * _drawerWidth
        });
    }
    ...
    function removeLoadingMessage() {
        TweenMax.to($("#initialization-cover"), 1, {
            alpha: 0,
            ease: Quad.easeOut,
            onComplete: function() {
                $("#initialization-cover").remove();
            }
        }), TweenMax.to($(".initialization-message"), 2, {
            top: "+=50px",
            ease: Quad.easeIn,
            onComplete: function() {
    ...
    function initialize() {
        ...
        _eventDispatcher = APP.EventDispatcher, _isVisible = !0, _modalCoverEl = $("#modal-cover"), 
        _modalBackgroundEl = $(".modal-background"), _modalCloseButtonEl = $(".modal-close-button");
    ...
    var _containerEl, _children = [], _counter = 0, _defaultExpireDuration = 7e3, _templateToast = _.template('<div class="toast" id="<%= id %>"><div class="content"><h1><%= title %></h1><p><%= message %></p></div><div class="controls"><button><i class="fa fa-close"></i></button></div></div>');
    return {
        initialize: initialize,
        add: add,
        remove: remove
    ...
    methods: {
        ...
        render: function() {
            var templateHTML = '<div class="item"><ul class="item-content <%= categories[0] %>" data-value="<%= id %>"><li class="image"><div class="image-wrapper"><img src="<%= previewImage %>"></div></li><ul class="data"><li class="title"><%= title %></li><ul class="metadata"><li class="left"><% _.each(categories, function(cat) { %><i class="fa fa-cube"></i><%= cat %><% }); %></li><li class="right"><i class="fa fa-puzzle-piece"></i><%= complexity %></li></ul></ul></li></ul></div>';
            this.template = _.template(templateHTML), this.renderedHTML = this.template(this.data), 
            this.element = $(this.renderedHTML), this.elementContent = this.element.find(".item-content"), 
            this.dataEl = this.element.find(".data"), this.imageEl = this.element.find(".image-wrapper");
        },
        postRender: function() {
    ...
    function initialize(elID) {
        ...
        _itemDTemplateSrc = '<div class="content"><div class="title"><h1><%= title %></h1></div><div class="description"><div class="extras"><button id="item-details-share-button" class="basic-button share-button"><i class="fa fa-share-alt"></i><em>Share</em></button></div><div class="description-data"><ul><li class="lob icon-left"><i class="fa fa-building"></i>Created for <em><%= companyArea %></em></li><li class="date icon-left"><i class="fa fa-calendar"></i>Completed on <em><%= dateCompleted %></em></li><li class="duration icon-left"><i class="fa fa-clock-o"></i>The solution is <em><%= duration %></em> long</li><li class="complexity icon-left"><i class="fa fa-puzzle-piece"></i><em><%= complexity %></em> complexity</li></ul></div><div class="preview-images"><ul><% _.each(images, function(image) { %><li><div class="float-image"><img src="<%= image %>" alt="<%= title %> preview image"></div></li><% }); %></ul></div><%= description %><div class="description-metadata"><ul><% _.each(categories, function(cat) { %><li class="type icon-left"><i class="fa fa-cube"></i><%= cat %></li><% }); %><% _.each(tags, function(tag) { %><li class="type icon-left"><i class="fa fa-tag"></i><%= tag %></li><% }); %></ul></div><div class="content-links"><ul><% _.each(links, function(link) { %><li class="icon-left"><a href="<%= link %>" target="_blank"><i class="fa fa-external-link"></i><%= link %></a></li><% }); %></ul></div></div></div>', 
        _itemDTemplate = _.template(_itemDTemplateSrc), _messageTemplateSrc = '<div class="content"><div class="title"><h1><%= title %></h1></div><div class="description"><%= description %></div></div>', 
        _messageTemplate = _.template(_messageTemplateSrc);
    }
    function showItem(item) {
        _currentItem = item, _containerEl.html(_itemDTemplate(_currentItem)), _floatImageView.apply(_containerEl.find(".preview-images")), 
        _shareButtonEl = document.getElementById("item-details-share-button"), _shareButtonEl.addEventListener(APP.globals().mouseClickEvtStr, doShareAction, !1), 
    ...
    function doShareAction() {
        var shareStr = "mailto:?subject=I'm sharing: " + _currentItem.title + '&body=I thought you would like this ... <br><br><a href="' + document.location.href + '">' + _currentItem.title + "</a><br><br>" + _currentItem.description, shareWin = window.open(shareStr);
        shareWin.close();
    }
    ...
    function initialize() {
        _viewPortCoverEl = $(_coverDivID), _captionEl = _viewPortCoverEl.find(".caption"), 
    ...
    function apply(container) {
        getFloatingElementsInContainer(container).forEach(function(el) {
            $(el).wrapInner('<div class="float-image-wrapper" />'), el.addEventListener(APP.globals().mouseClickEvtStr, onImageClick, !1);
        });
    }
    ...
    var _viewPortCoverEl, _viewPortCoverClickStream, _captionEl, _currentImageElement, _coverDivID = "#float-image-cover", _floatingImageClass = ".float-image", _window = $(window);
    return {
        initialize: initialize,
        apply: apply,
        remove: remove
